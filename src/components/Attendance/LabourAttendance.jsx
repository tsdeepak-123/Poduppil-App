...
import { View, Text, Button, TouchableOpacity, Image, ScrollView, ActivityIndicator, TextInput } from 'react-native'; // Import TextInput
import { RadioButton } from 'react-native-paper';
import moment from 'moment';
import DateTimePicker from '@react-native-community/datetimepicker';
import { fetchLabourAttendanceList, updateLabourAttendance } from '../../services/api';
...
const LabourAttendance = () => {
  ...
  const getLabourAttendance = async () => {
    try {
      ...
      const data = await fetchLabourAttendanceList(formattedDate);
      setLabourData(data);
  ...
  const updateAttendance = async () => {
    try {
      if (Object.keys(selectedValues).length === 0) {
        setError('Please select attendance for at least one laborer.');
        return;
      }
      ...
      const formattedDate = moment(selectedDate).format("YYYY-MM-DD");
      const response = await updateLabourAttendance(selectedValues, formattedDate);

      if (response.success) {
        const remainingLaborers = labourData?.filter((labour) => !selectedValues.hasOwnProperty(labour._id));
        setLabourData(remainingLaborers);
        setSearchResults(remainingLaborers);
  ...
  return (
    <ScrollView>
      <View style={{ alignItems: 'center', marginBottom: 150 }}>
        ...
        {labourData.length !== 0 && <Button title="Submit" onPress={updateAttendance} color="blue" />}
      </View>
    </ScrollView>
  );
};
